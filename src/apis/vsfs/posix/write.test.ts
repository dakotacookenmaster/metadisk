...
describe("writes data to a file", () => {
    test("successfully writes data to an open file", async () => {
        ...
        await expect(write(fd, binaryStringToBuffer("1010101"))).resolves.toBeUndefined()
    })
    test("fails with an out-of-bounds file descriptor", async () => {
        await expect(write(-1, binaryStringToBuffer("1010101"))).rejects.toThrowError(InvalidFileDescriptorError)
        await expect(write(1000000, binaryStringToBuffer("1010101"))).rejects.toThrowError(InvalidFileDescriptorError)
    })
    test("fails to write data that exceeds the maximum file size", async () => {
        const fd = await open("/abc", [OpenFlags.O_CREAT, OpenFlags.O_RDWR], Permissions.READ_WRITE_EXECUTE)
        await expect(write(fd, binaryStringToBuffer("1".repeat(150000)))).rejects.toThrowError(FileOverflowError)
    })
    test("successfully reuses a block when new data of the same size is written", async () => {
        const fd = await open("/abc", [OpenFlags.O_CREAT, OpenFlags.O_RDWR], Permissions.READ_WRITE_EXECUTE)
        await write(fd, binaryStringToBuffer("1010101"))
        await expect(write(fd, binaryStringToBuffer("11110000"))).resolves.toBeUndefined()
    })
    test("successfully deallocates blocks when a smaller write is requested", async () => {
        const fd = await open("/abc", [OpenFlags.O_CREAT, OpenFlags.O_RDWR], Permissions.READ_WRITE_EXECUTE)
        await write(fd, binaryStringToBuffer("1".repeat(50000)))
        await expect(write(fd, binaryStringToBuffer("11110000"))).resolves.toBeUndefined()
    })
    ...
    test("fails if the file doesn't have write permissions", async () => {
        ...
        await expect(write(fd, binaryStringToBuffer("101010"))).rejects.toThrowError(AccessDeniedError)
    })
})