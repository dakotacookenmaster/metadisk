...
describe("read a block from the disk", () => {
    test("should throw an error when a block is outside of the accepted range", async () => {
        await expect(writeBlock(100, new Uint8Array(64))).rejects.toThrowError(InvalidBlockAddressError)
        await expect(writeBlock(-1, new Uint8Array(64))).rejects.toThrowError(InvalidBlockAddressError)
    })
    test("should successfully write a valid block", async () => {
        await expect(writeBlock(4, new Uint8Array(64))).resolves.toBeTruthy()
    })
    test("should fail if the write size is larger than a block", async () => {
        await expect(writeBlock(4, new Uint8Array(2050))).rejects.toThrowError(BlockOverflowError)
    })
    test("should call the progress callback if it's provided", async () => {
        const progressCb = vi.fn()
        await expect(writeBlock(4, new Uint8Array(64), progressCb)).resolves.toBeTruthy()
