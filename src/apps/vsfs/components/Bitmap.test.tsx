...
import { binaryStringToBuffer } from "../../../apis/utils/BitBuffer.utils"
    ...
    test("256 or less should render on a single page", async () => {
        const { queryByTestId } = render(
            <Provider store={store}>
                <Bitmap setSelected={() => {}} setSelectedInode={() => {}} setBlockNumber={() => {}} data={binaryStringToBuffer("0000")} type="inode" />
            </Provider>,
        )
        // Expect 8 bitmap boxes to appear (4 bits rounds up to 1 byte = 8 bits)
        for (let i = 0; i < 8; i++) {
            const box = queryByTestId(`bit-${i}`)
            expect(box).toBeInTheDocument()
        }

        // There shouldn't be a ninth bit
        const box = queryByTestId(`bit-8`)
            ...
            render(
                <Provider store={store}>
                    <ThemeProvider theme={theme}>
                        <Bitmap setSelected={() => {}} setSelectedInode={() => {}} setBlockNumber={() => {}} data={binaryStringToBuffer("1" + "0".repeat(10000))} type="data" />
                    </ThemeProvider>
                </Provider>,
            )
